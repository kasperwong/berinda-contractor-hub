...
type Profile = { role?: "admin" | "editor" | "viewer"; active?: boolean; companyName?: string; status?: string; email?: string; displayName?: string; phone?: string };
export const AuthProfileContext = createContext<Profile | null>(null);
export function useAuthProfile() { return useContext(AuthProfileContext); }
...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  const profileWithIdentity = { ...profile, email: user.email ?? profile.email ?? "", displayName: profile.displayName ?? user.displayName ?? "" };
  return <AuthProfileContext.Provider value={profileWithIdentity}><>{children}{showAccountMenu && <div className="account-menu-popover"><div><strong>{user.email}</strong><small>{profile.role} access</small></div>{profile.role === "admin" && <button onClick={() => { setShowAdmin(true); setShowAccountMenu(false); }}>User management</button>}<button onClick={() => signOut(auth)}>Sign out</button></div>}{showAdmin && <AdminUserPanel auth={auth} db={db} onClose={() => setShowAdmin(false)} />}</></AuthProfileContext.Provider>;
}
