...
export function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return <><div className="account-bar"><span>Signed in as <strong>{user.email}</strong> · {profile.role}</span><div>{profile.role === "admin" && <button onClick={() => setShowAdmin(true)}>User management</button>}<button onClick={() => signOut(auth)}>Sign out</button></div></div>{children}{showAdmin && <AdminUserPanel auth={auth} db={db} onClose={() => setShowAdmin(false)} />}</>;
}
