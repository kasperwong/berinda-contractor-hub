...
function ContractorHubApp() {
  ...
  const filtered = useMemo(() => {
    ...
    return contractorRows.filter((contractor) => {
      ...
      return matchesSearch && matchesStatus && matchesTrade && matchesLocation && nameMatch && tradeColumnMatch && contactMatch && gradeMatch && scoreMinMatch && scoreMaxMatch && columnStatusMatch && approvalMatch && projectsMatch && groupProjectsMatch;
    });
                  ...
                  {sortedContractors.map((contractor) => {
                    ...
                    return (
                      <tr key={contractor.id} className={isActive ? "active-row" : ""} onClick={() => setActiveContractor(contractor)}>
                        ...
                        <td><div className="directory-detail trade-cell"><strong>{contractorTrades(contractor).join(" · ")}</strong></div></td>
          ...
          <section className="modal project-table-modal" role="dialog" aria-modal="true" aria-labelledby="project-list-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="full-project-table-wrap">
              <table className="full-project-table">
                <thead><tr><th>Select</th><th><button className="sortable-heading" onClick={() => sortProjects("name")}>Project Name{sortIndicator("name")}</button></th><th>Scope</th><th>Building Type</th><th>Developer</th><th>Client / Main Contractor</th><th>Location</th><th><button className="sortable-heading" onClick={() => sortProjects("value")}>Contract Value RM{sortIndicator("value")}</button></th><th>Commencement Date</th><th>Completion Date</th><th>Status</th><th>Progress</th></tr></thead>
                <tbody>{sortedPopupProjects.map((project) => { const selected = selectedProjects.includes(project.id); const dates = project.period.split(" – "); return <tr key={project.id}><td><button className={`row-check ${selected ? "checked" : ""}`} onClick={() => toggleProject(project.id)} aria-label={`${selected ? "Remove" : "Select"} ${project.name}`}>{selected ? "✓" : ""}</button></td><td><strong>{project.name}</strong></td><td><span className="group-project-scope">{project.scope}</span></td><td>{project.projectType ?? "-"}</td><td>{project.developer ?? "-"}</td><td>{project.client || "-"}</td><td>{project.location || "-"}</td><td><strong>{money(project.value)}</strong></td><td>{project.commencementDate ?? dates[0] ?? "-"}</td><td>{project.completionDate ?? dates[1] ?? "-"}</td><td><span className={`table-project-status ${project.status.toLowerCase()}`}>{project.status}</span></td><td>{project.progress ?? "-"}</td></tr>; })}</tbody>
              </table>
      ...
      {showAddContractor && (
        <div className="modal-backdrop nested-modal" role="presentation" onMouseDown={() => setShowAddContractor(false)}>
          <form ref={addContractorFormRef} className="modal form-modal add-contractor-modal" onSubmit={handleAddContractor} onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="required-fields-note"><strong>Only two fields are required</strong><span>Enter the legal company name and one or more trades. The remaining information can be added later.</span></div><div className="form-grid"><label className="wide">Company name<input name="name" required placeholder="Legal company name" /></label><label className="wide">Trade(s)<input name="trade" required placeholder="e.g. Landscape, Civil works, Piling & foundation" /><small>Separate multiple trades with commas.</small></label></div><div className="form-grid"><label>Contact name<input name="contactName" placeholder="Mr / Ms and name" /></label><label>Mobile / handphone number<input name="mobile" type="tel" /></label><label>Office number<input name="officePhone" type="tel" /></label><label className="wide">Email address<input name="email" type="email" placeholder="company@example.com" /></label><label>Pre-Q date<input name="preqDate" type="date" /></label><label>Pre-Q score<input name="score" type="number" min="0" max="100" defaultValue="0" /></label><label>Approval date<input name="approvalDate" type="date" /></label><label>CIDB grade<select name="grade" defaultValue="Not provided"><option>Not provided</option><option>G7</option><option>G6</option><option>G5</option><option>G4</option></select></label><label>Location<select name="location" defaultValue="Not provided"><option>Not provided</option><option>Johor</option><option>Selangor</option><option>Kuala Lumpur</option><option>Other</option></select></label></div>
